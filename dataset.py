...
class Starburst_augment(object):
    ...
    def __call__(self, img):
        ...
        # 保证starburst尺寸与目标区域匹配
        target_height = 549 - 92
        if starburst.shape[0] != target_height:
        ...
        # 计算实际使用的区域，确保不超出img边界
        start_y = max(92 + y, 0)
        end_y = min(549 + y, img.shape[0])

        # 适用于starburst和目标img区域的实际高度
        actual_starburst_height = end_y - start_y
        if actual_starburst_height > 0:  # 确保有实际的覆盖区域
            img[start_y:end_y, 0:400] = img[start_y:end_y, 0:400] * (
                        (255 - starburst[:actual_starburst_height, :]) / 255) + starburst[:actual_starburst_height, :]
...
class Line_augment(object):
    def __call__(self, base):
        ...
        return Image.fromarray(aug_base.astype(np.uint8))
